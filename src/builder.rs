...
use floresta_wire::{
    address_man::AddressMan, mempool::Mempool, node::UtreexoNode, node_interface::NodeInterface, running_node::RunningNode,
    UtreexoNodeConfig,
};
...
/// [`FlorestaBuilder`] builds a node from the `UtreexoConfig` provided, or builds a node from default values.
...
impl Default for FlorestaBuilder {
    fn default() -> Self {
        ...
        // is set, the node will validate the previous history in the background.
        //
        // Currently, only `Network::Bitcoin` is supported for AssumeUTreeXO.
        let assume_utreexo_default: AssumeUtreexoValue = ChainParams::get_assume_utreexo(network_default);
        ...
        //
        // TODO(@luisschwab): make this deterministic based on the release version + `floresta-wire` version.
        ...
        Self {
            ...
            config: UtreexoNodeConfig {
                ...
                // TODO(@luisschwab): set this to true when implementing wallet rescan with CBF.
...
impl FlorestaBuilder {
    ...
    ///
    /// TODO(@luisschwab): what happens if we set a config to an already running node?
    ...
    pub async fn build(self) -> Result<FlorestaNode, BuilderError> {
        ...
        // Try to load an existing chain store from disk, or create a new chain store.
        ...
        let chain_state: Arc<ChainState<FlatChainStore>> = Arc::new(ChainState::new(chain_store, self.config.network, self.assume_valid));

        info!("initialized chainstore at {}", self.config.datadir);

        // Create a signal that keeps track of whether [`FlorestaNode`] should stop.
        let stop_signal: Arc<RwLock<bool>> = Arc::new(RwLock::new(false));

        let (sender, _) = oneshot::channel();

        // Create a `Pollard` accumulator to be used for our transaction's mempool.
        let pollard_acc: Pollard<BitcoinNodeHash> = Pollard::new();
        let mempool: Arc<Mutex<Mempool>> = Arc::new(Mutex::new(Mempool::new(pollard_acc, 1_000_000)));

        let node = UtreexoNode::<_, RunningNode>::new(
            self.config.clone(),
            chain_state.clone(),
            mempool,
            None, // TODO(@luisschwab): update this once CBF scanning is implementing.
            stop_signal.clone(),
            AddressMan::default(),
        )
        ...
        let sigint_task: JoinHandle<()> = {
            ...
            let chain_state: Arc<ChainState<FlatChainStore>> = chain_state.clone();

            task::spawn(async move {
                tokio::signal::ctrl_c().await.expect("failed to initialize SIGINT handler");
